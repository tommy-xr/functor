...
| `Animator` | bundled crossfade state over `Anim` — engine hosts only, and NOT in the generated reference (documented below) |
...
`functor-prelude/prelude/*.funi`, one file per module, each with `//!` module
prose and `///` per-function docs. (VSCode's go-to-definition on `Scene.cube`
opens exactly that file at the signature.)
...
| `Net` | built-in message types for `Effect.httpGet`/`httpPost` and `Sub.connect`/`listen` (see below — NOT in the generated reference) |
...
**Assets are branded values, not paths** (the B.6 flag day): `Scene.model`,
`Sprite.image`, `Effect.play`/`playAt`/`playThen`, and
`AudioSource.ambient`/`at` take `Assets.*` constants (generated by
`functor import`) or `Asset.model`/`texture`/`sound(…)` at a data boundary — a
bare path string is a check error. `Texture.file` paths, `Skybox.files` faces,
`Anim.clip` names, and `AudioSource` KEYS are not asset locators and stay
strings. `functor import` also generates typed clip and joint constants
