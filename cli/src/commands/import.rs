...
//! a game (or sibling module) declaring its own record type with the same field
//! set as `Clip` or a generated `<Model>Clips` makes those literals ambiguous —
//! a loud load error ("ambiguous record literal … annotate") fixed by renaming
//! a field of the user type.
...
/// First line of every generated file — the marker that lets a rerun
/// distinguish (and safely remove) its own output from a hand-written file.
...
which collide with the generated Clip record's fields",
                ...
                let dups = duplicate_clip_names(&clips);
                if !dups.is_empty() {
                    emit(Event::Warning {
                        message: format!(
                            "{}: duplicate clip name(s) {} — Anim.clip plays the first \
match, so only the first of each is generated",
    ...
    match generate_assets_source(&models) {
        ...
        None => {
            // No usable clips. Remove a previously GENERATED file so stale
            // constants for since-removed assets can't linger (a hand-written
            // assets.fun without the marker is left alone).
            let out = dir.join(ASSETS_FILE);
            let previously_generated = fs::read_to_string(&out)
                .map(|s| s.starts_with(GENERATED_HEADER))
                .unwrap_or(false);
            if previously_generated {
                fs::remove_file(&out)?;
                emit(Event::Info {
                    message: format!(
                        "no usable animation clips in the scanned models — removed stale {}",
                        out.display()
                    ),
                });
            } else {
                emit(Event::Info {
                    message: format!(
                        "no usable animation clips in the scanned models — {} not written",
                        ASSETS_FILE
                    ),
                });
            }
...
/// the one `Anim.clip`'s first-match lookup actually plays — so every
/// generated constant is addressable.
...
pub fn generate_assets_source(models: &[ModelClips]) -> Option<String> {
    ...
    for (stem, clips) in models {
        ...
        // two models whose fields order differently (clip sorting is by
        // original name, pre-sanitization) must still share one type — two
        // same-shaped declarations would make every matching literal
        // ambiguous.
...
";
    ...
    fn same_field_set_in_different_order_still_shares_the_type() {
        ...
        // sanitized field set differently per model ("Idle" < "run" but
        // "Run" < "idle"). The type key must be the sorted SET — emitting two
        // same-shaped types would make every matching literal ambiguous.
        let models = vec![
            (
                "abot".to_string(),
                vec![("Idle".to_string(), 1.0), ("run".to_string(), 2.0)],
            ),
            (
                "bbot".to_string(),
                vec![("idle".to_string(), 3.0), ("Run".to_string(), 4.0)],
            ),
        ];
        let src = generate_assets_source(&models).unwrap();
        assert_eq!(src.matches("type ").count(), 2, "Clip + ONE shared model type:\n{src}");
    ...
    fn duplicate_clip_names_keep_only_the_first() {
        ...
        let src = generate_assets_source(&[("bot".to_string(), clips)]).unwrap();
