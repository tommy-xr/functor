...
const GAME_3D: &str = include_str!("../../templates/3d/game.functor");
const GAME_FPS: &str = include_str!("../../templates/fps/game.functor");
...
mod tests {
    ...
    fn three_d_template_creates_a_typechecked_project() {
        ...
        assert_eq!(
            fs::read_to_string(directory.0.join("game.functor")).unwrap(),
            GAME_3D
        );
    ...
    fn unrelated_files_are_preserved() {
        ...
        assert!(directory.0.join("game.functor").is_file());
    }
    ...
    fn a_conflict_does_not_overwrite_or_partially_initialize() {
        ...
        fs::create_dir_all(&directory.0).unwrap();
        fs::write(directory.0.join("game.functor"), "user source\n").unwrap();

        let error = execute(&directory.0, &Template::Fps).unwrap_err();

        assert_eq!(error.kind(), std::io::ErrorKind::AlreadyExists);
        assert!(error.to_string().contains("game.functor"));
        assert_eq!(
            fs::read_to_string(directory.0.join("game.functor")).unwrap(),
            "user source\n"
        );
