...
pub enum DebugRequest {
    ...
    /// (body = the raw `.functor` text), preserving the model. Ok carries a status
                ...
                (Method::Get, "/") => {
                    ...
                    let body = serde_json::json!({
                        ...
                        "endpoints": {
                            ...
                            "POST /reload-source": "swap game logic from the request body (raw .functor source), model preserved — 400 with the load error on a broken push",
