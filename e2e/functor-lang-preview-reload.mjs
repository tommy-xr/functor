...
await page.evaluate(() => {
  ...
  window.addEventListener("message", (e) => {
    if (e.data && e.data.type === "functor-lang-set-source-result") window.__results.push(e.data);
  });
});
const push = async (source) => {
  const before = await page.evaluate(() => window.__results.length);
  await page.evaluate((s) => window.postMessage({ type: "functor-lang-set-source", source: s }, "*"), source);
...
const errCount = () => consoleLog.filter((m) => m.includes("[functor-lang] tick error")).length;
