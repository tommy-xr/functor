// mpclient — the Functor Lang port of examples/mpclient (roadmap E2, docs/functor-lang.md).
//
...
//
// Port notes (same as mpserver): Functor Lang has no `%` (mod4 recursion picks the
...
// color in every pane. Functor Lang has no `%`, so mod4 wraps by recursion.
