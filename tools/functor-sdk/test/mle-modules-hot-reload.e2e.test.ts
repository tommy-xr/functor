...
// B8 part 1, end-to-end (docs/mle.md): a multi-file MLE project where the
...
test(
  ...
  async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "mle-modules-"));
    const entryPath = join(dir, "game.mle");
    const configPath = join(dir, "config.mle");
