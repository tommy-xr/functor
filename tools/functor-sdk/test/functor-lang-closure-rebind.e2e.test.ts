...
// B5 part 2 end-to-end (docs/functor-lang.md): a closure STORED IN THE MODEL rebinds
...
test(
  ...
  async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "functor-lang-rebind-"));
    const functorLangPath = join(dir, "game.fun");
    writeFileSync(functorLangPath, game("k * dt"));
    ...
    writeFileSync(functorLangPath, game("k * dt * 10.0"));
    await waitFor(
      async () => runner.logs(),
