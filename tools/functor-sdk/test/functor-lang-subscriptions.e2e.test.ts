...
// C4b-2 end-to-end (docs/functor-lang.md): subscriptions fire on the global time
...
test(
  ...
  async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "functor-lang-subs-"));
    const functorLangPath = join(dir, "game.functor");
    writeFileSync(functorLangPath, unsubscribed);
    ...
    writeFileSync(functorLangPath, subscribed);
    await waitFor(
      async () => runner.logs(),
