...
A fully static site: landing page (whose hero background is `examples/hero.functor`
...
- `player.html` — the runtime host page; the sibling of the CLI dev server's
  `index-functor-lang.html`, but the `.functor` entry comes from `?game=`. Keep its input
...
- `build.mjs` copies selected repo examples' `game.functor` sources (see the map in `build.mjs`)
