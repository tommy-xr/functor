...
import { galleryCard, renderGalleryCards } from "./src/gallery-html.ts";
...
// The games carousel's cards, derived ONCE: the landing page's markup, the
// emitted examples/gallery.json and the build's own "is the box art there?"
// check all read these, so the check can't pass on paths the cards don't use.
// `order` only sequences them, so it is not carried into the card itself.
const galleryOrders = new Set(GALLERY.map((example) => example.gallery.order));
if (galleryOrders.size !== GALLERY.length) {
  console.error("two gallery examples in src/examples.ts declare the same `order`");
  process.exit(1);
}
const galleryCards = GALLERY.map(galleryCard);

/**
 * Fill a `<!--marker-->` slot in a page with prerendered markup. Exactly one
 * slot each: a second occurrence (say, in a comment above the real slot) would
 * take the markup and leave the real one empty, since a string `replace` only
 * rewrites the first match.
 */
const fillSlot = (html, page, marker, markup) => {
  const token = `<!--${marker}-->`;
  const occurrences = html.split(token).length - 1;
  if (occurrences !== 1) {
    console.error(`${page} has ${occurrences} ${token} prerender markers — expected exactly 1`);
    process.exit(1);
  }
  return html.replace(token, () => markup);
};
...
for (const page of PAGES) {
  ...
  if (page.endsWith(".html")) {
    ...
    if (page === "index.html") {
      ...
      // cards can't disagree with the media.
      html = fillSlot(html, page, "games-gallery", renderGalleryCards(galleryCards));
    }
    if (page === "docs/index.html") {
      for (const [marker, markup] of Object.entries(PRERENDER)) {
        html = fillSlot(html, page, marker, markup);
      }
...
// `order` sequences the cards and is not re-emitted — the array is already in it.
// The media is committed (site/media/box-art, generated by `npm run demo:box-art`)
// and copied above. A gallery entry whose box art was never captured would ship a
// card with two 404s and a green build — refuse instead.
for (const card of galleryCards) {
  for (const asset of [card.poster, card.animation]) {
    try {
      await access(`${dist}/${asset}`);
    } catch {
      console.error(
        `gallery entry "${card.id}" has no ${asset} — capture it with ` +
          `\`node site/demos/box-art.mjs ${card.id}\``
      );
      process.exit(1);
    }
  }
}
await writeFile(`${dist}/examples/gallery.json`, JSON.stringify(galleryCards, null, 2) + "\n");
