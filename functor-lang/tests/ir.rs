//! B2 verification (docs/functor-lang.md): IR snapshot goldens per example,
...
fn lower_src(src: &str) -> Module {
    ...
    match functor_lang::lower(program) {
        Ok(module) => module,
        Err(err) => {
...
fn lower_err(src: &str) -> (String, usize, usize) {
    let program = functor_lang::parse(src).expect("source should parse");
    let err = functor_lang::lower(program).expect_err("source should fail to lower");
    let (line, col) = functor_lang::line_col(src, err.span.start);
...
fn lambda_body(module: &Module, index: usize) -> (&[functor_lang::ir::Param], &Expr) {
