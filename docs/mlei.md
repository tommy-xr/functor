# MLE interface files (`.mlei`) — design

**Status:** design / not yet implemented. **Owner:** language.
**Related:** `docs/mle.md` (roadmap), the `mle-language` skill (syntax source of
truth — update it alongside each slice), `mle/src/project.rs` (module loading),
`mle/src/types.rs` (checker, `builtin_signature`).

## Goal

Give the MLE typechecker **real types for the host prelude** (`Scene.*`,
...
agent that can read `scene.mlei` knows the signatures. That is worth more than a
...
Each is independently shippable and updates the `mle-language` skill in the same
change.
...
- **Return-position rule:** a function type used directly as a **lambda return
  annotation** must be parenthesized — `(): ((A) => B) => body` — because MLE
...
`mle/examples/{tuples,lists,strings}.mle`) to `(A, B)`, remove `*` from the type
...
### 2d — `val` signatures + `.mlei` grammar + interface-only modules

The interface file itself. A `.mlei` contains **only** declarations — no bodies:
...
- **`val name : Type`** — a name bound to a type, no body; the implementation
  lives elsewhere (the Rust host, or — later — a paired `.mle`).
- **`.mlei` grammar** is the `.mle` grammar restricted to `type` (concrete or
  abstract) and `val` items — a `let` with a body in a `.mlei` is an error.
- **Interface-only modules (the key module enhancement):** the project loader
  accepts a module defined *solely* by a `.mlei` (no `.mle`). Its `type`/`val`
...
- **Runtime is unchanged:** host-module `val`s still resolve to host values via
  the existing External→host path at run time; `.mlei` is a *check-time overlay*.
  A user `.mlei` `val` with no host backing and no `.mle` body is a runtime
  "unknown external" if actually called (existing error) — acceptable.

*Verify:* a scratch project with a hand-written `foo.mlei` (no `foo.mle`) gives
`Foo.bar` a real type in `check`/hover; a `let … = …` body in a `.mlei` is a load
error; `open` and qualified access both resolve.

### 2e — Ship the host prelude `.mlei` + drift test

Author the FunctorHost surface as `.mlei` files (`scene.mlei`, `camera.mlei`,
`physics.mlei`, …), **bundle them into the `mle` crate** (`include_str!`), and
load them by default whenever the checker runs — so host-awareness works
everywhere (`mle check`, the LSP) with **no config**. This flips the
...
by a `.mlei` with no `.mle`). Everything else reuses existing machinery:
...
- **Interface *checking* of a paired `.mle`** (OCaml `.mli` model: the impl must
  ...
  *encapsulation* — valuable, but not needed for host types (host modules have no
  `.mle`). Design `.mlei` so it slots in later; don't build it now.
- **Scope ergonomics for host types** — whether common prelude types
  ...
  the grammar work.
- **Unifying builtins into `.mlei`** — the 17 builtins (`List.map`, …) get types
  from Rust `builtin_signature` today. Once `.mlei` exists they *could* move to a
  bundled `builtins.mlei` for one declarative mechanism. A nice follow-up, not
  required.
...
2. **Signature form: `val` vs bodyless `let`.** **LEANING: bodyless `let`** — since
   ...
   `= value` dropped (`let cube : () => SceneNode`), so a `.mlei` is "the `.mle`
   with bodies erased" — no new keyword. Bodies are required in `.mle`, forbidden
   in `.mlei`. (Alternative: explicit `val`, clearer decl-vs-def, matches
...
5. **`.mlei` beside `.mle`** — for host modules there's only `.mlei`. If a user
   ships both `foo.mle` and `foo.mlei`, that's the deferred encapsulation feature;
   until then, define the loader behavior (ignore the `.mlei`, or error).
