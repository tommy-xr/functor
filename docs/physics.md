...
networked physics (7a/7b — the `examples/orbs` demo), and cross-target
...
examples (`hello`, `examples/orbs`) — they destructure `{ time }` and are
otherwise untouched.
...
any split-ownership scene (including the `examples/orbs` demo below) hits
...
### Networked-physics demo: `examples/orbs`

The concrete vehicle for Phase 7 — grown directly from the existing examples
(today its `module Server` broadcasts the whole world as a typed `Snapshot` and
its `module Client` renders the last one it was sent). The scene: **each client owns a bouncing ball; the server owns the
...
| **7a. Networked physics (state-sync)** | `Authority`, `examples/orbs` grown to client-owned balls + server-owned objects, kinematic `Remote` + interpolation. No prediction. | both |
